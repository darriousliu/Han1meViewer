...
/**
 * iOS 用 ImageIO 的缩略图接口做采样解码——`kCGImageSourceThumbnailMaxPixelSize`
 * 是系统原生的下采样，不会把原图整张解进内存；`kCGImageSourceCreateThumbnailWithTransform`
 * 顺带按 EXIF 把方向转正。
 *
 * 最后那一段 `CGImage → JPEG → skia Image` 看着绕，但它发生在**已经缩小之后**的图上，
 * 代价可以忽略；这是把 CoreGraphics 的位图交给 Compose 最短的一条路
 * （Compose 侧只认 skia 的 `Image`/`Bitmap`）。
 */
...
actual suspend fun decodeSampledImageBitmap(
    ...
    // ObjC 对象要显式 toll-free bridge 成 CF 指针才能喂给 CF 接口，
    // CFBridgingRetain 会 +1 引用，用完各自 CFRelease
