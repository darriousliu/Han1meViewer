...
import io.github.darriousliu.han1meviewer.LOCAL_DATE_FORMAT
...
object Parser {
    ...
    fun homePageVer2(body: String): WebsiteState<HomePage> {
        ...
        Logger.i(tag = "userInfo") { "name:$username;id:$userId" }
    ...
    private fun hanimeSearchNormalVer2(
        ...
        Logger.d(tag = "search_result") { "$hanimeSearchList" }
    ...
    fun hanimeVideoVer2(body: String): VideoLoadingState<HanimeVideo> {
        ...
        var likeStatus = parseBody.selectFirst("[name=like-status]")
            ?.attr("value")
        Logger.i(tag = "likeStatus") { likeStatus.toString() }
            ...
            if (playlistScroll != null) {
                ...
                if (children.firstOrNull()?.hasClass("playlist-hover-wrap") == true) {
                    ...
                    children.forEach { parent ->
                        ...
                        val eachTitleCover =
                            cardMobilePanel?.select("div > div > div > img")?.getOrNull(1)
                        val eachIsPlaying = cardMobilePanel?.select("div > div > div > div")
                            ?.firstOrNull()
                            ?.text()
                            ?.contains("播放") == true
                        val cardMobileDuration =
                            cardMobilePanel?.select("div[class*=card-mobile-duration]")
                        ...
                        val infoBoxes =
                            cardMobilePanel?.select("div.card-mobile-duration.card-playlist-large")
...
//                    Logger.i("children") { each.toString() }
...
//                }
        ...
        Logger.d(tag = "related_anime_list") { relatedAnimeList.toString() }
    ...
    fun comments(body: String): WebsiteState<VideoComments> {
        // ⚠️ 不能用 toString()：kotlinx 的 JsonPrimitive.toString() 给的是带引号的 JSON
        // 表示，而 org.json 的 get().toString() 给的是字符串内容本身。要用 .content。
        ...
        buildList {
            ...
            val reportableType =
                child.select("span.report-btn").first()?.attr("data-reportable-type")
        ...
        Logger.d(tag = "commentList") { commentList.toString() }
        return WebsiteState.Success(
            VideoComments(
                commentList,
                currentUserId,
...
//        }
    ...
    fun getMySubscriptions(body: String): WebsiteState<MySubscriptions> {
        ...
        Logger.i(tag = "getMySubscriptions") { "MaxPageList=$maxPage" }
        ...
        val videos = subscriptionsVideosRoot.select("div[class^=video-item-container]")
            .mapNotNull { videoCard ->
                try {
                    val link =
                        videoCard.selectFirst("a[class^=video-link]")?.absUrl("href")
                            ?: return@mapNotNull null
                    val videoCode = Regex("""watch\?v=(\d+)""").find(link)?.groupValues?.get(1)
                        ?: return@mapNotNull null
                    val coverUrl =
                        videoCard.select("img[class^=main-thumb]").getOrNull(0)?.absUrl("src")
                            ?: return@mapNotNull null
                    val title = videoCard.attr("title").trim()
                    val durationAndViews = videoCard.select("div[class^=thumb-container]")
                    val duration = durationAndViews.select("div[class^=duration]").text()
                    val views =
                        durationAndViews.select("div[class^=stat-item]").getOrNull(1)?.text()
                    ...
                    if (artistAndUploadTime.contains("•")) {
                        ...
                        artist = subtitleElement?.selectFirst("a")?.text()?.trim().orEmpty()
                        uploadTime =
                            subtitleElement?.selectFirst("span.subtitle-time, span")?.text()?.trim()
                                .orEmpty()
                    }
    ...
    private fun <T> T?.logIfParseNull(
        ...
        if (it == null) {
            if (loginNeeded) {
                if (isAlreadyLogin) {
                    Logger.d(tag = "Parse::$funcName") { "[$varName] is null. 而且處於登入狀態，這有點不正常" }
                }
            } else {
                Logger.d(tag = "Parse::$funcName") { "[$varName] is null. 這有點不正常" }
            }
        }
    }
}
