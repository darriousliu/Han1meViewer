...
) : ReadOnlyProperty<Any?, MutableStateFlow<V>> {
    private var cache: MutableStateFlow<V>? = null

    override fun getValue(thisRef: Any?, property: KProperty<*>): MutableStateFlow<V> =
        cache ?: MMKVFlow(
...
/**
 * 读走 MMKV、写的同时落盘的 [MutableStateFlow]。
 */
class MMKVFlow<V>(
