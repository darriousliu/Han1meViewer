...
/**
 * JDK 自带的 `ImageReadParam.setSourceSubsampling` 就是真·子采样读取，
 * 不需要第三方库：`reader.getWidth(0)` / `getHeight(0)` 只读文件头不分配像素，
 * 拿到尺寸算完采样率再 `reader.read(0, param)`。
 */
        ...
        ImageIO.createImageInputStream(ByteArrayInputStream(bytes)).use { input ->
            ...
            try {
                ...
                val sample = computeSampleSize(
                    reader.getWidth(0), reader.getHeight(0), maxDimension
                )
...
actual suspend fun encodeJpeg(bitmap: ImageBitmap, quality: Int): ByteArray? =
    withContext(Dispatchers.IO) {
        runCatching {
            // JPEG 没有 alpha 通道，带 alpha 的 BufferedImage 直接写会得到全黑图，
            // 先重绘到 TYPE_INT_RGB 上
            ...
            // ImageIO.write 用不了 quality，要自己拿 writer 配 ImageWriteParam
