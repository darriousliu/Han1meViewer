 ...
 * Android 支持全部设置项。
 *
 * 唯一按版本判断的是 [HomeSettingsCapabilities.dynamicColor]——它原来是
 * `HomeSettingsUiState.dynamicColorEnabled`，逐字搬过来的，行为不变。
 *
 * [HomeSettingsCapabilities.deepLinkSettings] 和 [HomeSettingsCapabilities.predictiveBack]
 * **故意不按版本判断**：前者在低版本上原本就是「显示出来、点了提示不支持」，
 * 后者在 UI 上本来就是 `enabled = false` 的占位。把它们改成按版本隐藏是行为变更，
 * 不属于本轮的搬迁范围；这两个 flag 在这里只负责「别的平台不显示」。
 */
