package io.github.darriousliu.han1meviewer.worker
...
import io.github.darriousliu.han1meviewer.core.notification.DOWNLOAD_NOTIFICATION_CHANNEL
import io.github.darriousliu.han1meviewer.core.common.EMPTY_STRING
import io.github.darriousliu.han1meviewer.core.storage.HFileManager
import io.github.darriousliu.han1meviewer.core.storage.HFileManager.createVideoName
...
import io.github.darriousliu.han1meviewer.util.saveTo
...
class HanimeDownloadWorker(
    ...
    private suspend fun requestContentLength(useHead: Boolean): Long? {
        return try {
            val response = if (useHead) {
                HanimeNetwork.downloadClient.head(downloadUrl)
            } else {
                HanimeNetwork.downloadClient.get(downloadUrl) {
        ...
        return withContext(Dispatchers.IO) {
            ...
            try {
                ...
                while (downloadedLength < entity.length) {
                    ...
                    HanimeNetwork.downloadClient.prepareGet(downloadUrl) {
                        if (requestNeedRange) {
                        ...
                        if (!canWrite) {
                            failureReason = response.toDownloadErrorMessage(requestNeedRange)
                            return@execute
                        }

                        val bodyStream = response.bodyAsChannel().toInputStream()
