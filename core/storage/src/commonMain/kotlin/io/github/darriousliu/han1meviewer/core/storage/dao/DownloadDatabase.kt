...
import io.github.darriousliu.han1meviewer.core.storage.dao.download.DownloadCategoryDao
...
abstract class DownloadDatabase : RoomDatabase() {
    ...
    object Migration1To2 : Migration(1, 2) {
        override suspend fun migrate(connection: SQLiteConnection) {
            ...
            connection.executeSQL("""DROP TABLE IF EXISTS HanimeDownloadedEntity""")
        }
    }
    ...
    object Migration2To3 : Migration(2, 3) {
        override suspend fun migrate(connection: SQLiteConnection) {
            ...
            connection.executeSQL("""CREATE INDEX IF NOT EXISTS `index_HanimeCategoryCrossRef_categoryId` ON `HanimeCategoryCrossRef` (`categoryId`)""")
            // Add coverUri column
            connection.executeSQL("""ALTER TABLE `HanimeDownloadEntity` ADD COLUMN `coverUri` TEXT NULL""")

            // Add state column with default value (convert from isDownloading)
            connection.executeSQL("""ALTER TABLE `HanimeDownloadEntity` ADD COLUMN `state` INTEGER NOT NULL DEFAULT ${DownloadState.Mask.UNKNOWN}""")
    ...
    object Migration3To4 : Migration(3, 4) {
        override suspend fun migrate(connection: SQLiteConnection) {
            ...
            connection.executeSQL("DROP TABLE `HanimeDownloadEntity`")
            connection.executeSQL("ALTER TABLE `HanimeDownloadEntity_new` RENAME TO `HanimeDownloadEntity`")
        }
    }
    object Migration4To5 : Migration(4, 5) {
        override suspend fun migrate(connection: SQLiteConnection) {
            ...
            connection.executeSQL("""CREATE INDEX IF NOT EXISTS `index_HanimeDownloadEntity_groupId` ON `HanimeDownloadEntity_new` (`groupId`)""")
...
internal fun buildDownloadDatabase(
