 ...
 * Android 上是导航到 WebView 过盾页跑一遍，iOS 以后可以接 WKWebView。
 */
...
object CloudflareChallengeHandler {
    /**
     * 平台的过盾实现，由 [platformCloudflareSolver] 提供。
     * 为 null 时插件直接放行 403，行为等同于没装这个插件。
     */
...
/**
 * 403 且 `cf-mitigated: challenge` → 让用户过盾 → 原样重发一次。
 *
 * 等用户过盾时真正挂起、不阻塞线程；加了把锁，多个并发请求同时撞到盾时只会拉起一次验证。
 */
